...
trait ExportTrait
{
    ...
    public function render() {
        ...
        // SAVE DATA
        $this->_exportData();
        $this->log(json_encode($this->_aResponse));
    ...
    private function _getCSVDataRow($aFields, $bHeader = false, $sDelimiter = self::EXPORT_DELIMITER) {
        ...
        foreach ($aFields as $iKey => $sValue) {
            $sExportField = $this->_aExportFields[$iKey] ?? null;

            /* CategoryPath (Ticket: #35896) */
            if (
                ($sExportField == 'CategoryPath')
                && ($sValue != 'CategoryPath')
            ) {
            ...
            if (
                ($sExportField == 'Attributes')
                || ($sExportField == 'ClonedAttributes')
            ) {
                $sValue = $this->_convertAttributes($sValue);
            }

            $sExportData = in_array($sExportField, $this->_aExportHtmlFields) ? $sValue : $this->_removeHtmlAndBlankLines($sValue);

            if ($bHeader && (self::PROCESS_CODE != 'FLOUR')) {
                $sExportData = $this->_getExportHeaderFieldName($sExportData);
            }
    ...
    private function _loadData() {
        ...
        // GET DATA
        $this->_sChannel = Registry::getRequest()->getRequestParameter('channel');
        $this->_iShopId = Registry::getRequest()->getRequestParameter('shop_id');
        $this->_iLang = Registry::getRequest()->getRequestParameter('lang');
...
//            $this->_aCsvData[] = $this->_getCSVDataRow($this->_aExportFields, true);
    ...
    protected function _getTmpExportDelimiter()
    {
        ...
        $aDelimiterMap = [
            'csv' => '|',
            'xml' => $this->_sTmpExportDelimiter,
        ];
        $sTmpExportDelimiter = $aDelimiterMap[$sTmpExportDelimiter] ?? $sTmpExportDelimiter;

        if (
            in_array(self::PROCESS_CODE, ['DOOFINDER', 'SOOQR'], true)
            && ($sTmpExportDelimiter === '' || $sTmpExportDelimiter === '|' || $sTmpExportDelimiter === self::EXPORT_DELIMITER)
        ) {
    ...
    private function _getPreparedExportFields($aFields = array())
    {
        ...
        foreach ($aFields as $iKey => $sField) {
            ...
            if (preg_match('/\\s+AS\\s+/i', $sField) === 1) {
                $aFields[$iKey] = $sField;
                continue;
            }
    ...
    private function _getExportHeaderFieldName($sField)
    {
        if (preg_match('/\\s+AS\\s+`?([^`]+)`?$/i', $sField, $aMatches) === 1) {
            return $aMatches[1];
        }
