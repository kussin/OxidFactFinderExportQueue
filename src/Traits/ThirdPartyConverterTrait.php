...
trait ThirdPartyConverterTrait
{
    ...
    {
        $sValue = $this->_sanitizeXmlValue($sValue);
    ...
    {
        return array(
            '_cdata' => $this->_sanitizeXmlValue($sValue, true),
        );
    }

    private function _sanitizeXmlValue($sValue, $bCData = false)
    {
        if (!is_scalar($sValue) && $sValue !== NULL) {
            return $sValue;
        }

        $sValue = (string) $sValue;
        $sValue = html_entity_decode($sValue, ENT_QUOTES | ENT_HTML5, 'UTF-8');
        $sValue = str_replace("\xc2\xa0", ' ', $sValue);
        $sValue = preg_replace('/[^\x{9}\x{A}\x{D}\x{20}-\x{D7FF}\x{E000}-\x{FFFD}]/u', '', $sValue);
