...
class wmdkffexport_queue extends oxubase
{
    ...
    private function _updateQueueData() {       
        ...
        if (
            ...
            $sManufacturer = $this->_getManufacturerTitle();
            ...
            $this->_aUpdateData['ClonedAttributes'] = $this->_cloneAttributes(
                ...
                $this->_getArticleFieldValue($this->_oProduct, 'wmdkvarselectmapping')
            );
            ...
            $this->_aUpdateData['Rating'] = $this->_getArticleFieldValue($this->_oProduct, 'oxrating');
            $this->_aUpdateData['RatingCnt'] = $this->_getArticleFieldValue($this->_oProduct, 'oxratingcnt');
            ...
            $modifiedDate = $this->_getArticleFieldValue($this->_oProduct, 'wmdkmodified');
            $this->_aUpdateData['DateModified '] = ($modifiedDate != '' && $modifiedDate != '0000-00-00') ? $modifiedDate : $this->_oProduct->oxarticles__oxinsert->value;
...
//        return $this->_sBaseUrl;
    ...
    private function _getManufacturerTitle() {
        $oArticle = ($this->_bIsVariant) ? $this->_oParent : $this->_oProduct;
        $oManufacturer = $oArticle ? $oArticle->getManufacturer() : null;

        if (!$oManufacturer || !isset($oManufacturer->oxmanufacturers__oxtitle)) {
            return '';
        }

        return (string) $oManufacturer->oxmanufacturers__oxtitle->value;
    }

    private function _hasManufacturer() {
        $oArticle = ($this->_bIsVariant) ? $this->_oParent : $this->_oProduct;

        return $oArticle && $oArticle->getManufacturer();
    }

    private function _getManufacturerLink() {
        if (!$this->_hasManufacturer()) {
            return $this->_getMainCategoryLink();
        }
    ...
    private function _hasHasNewFlag($bFalse = 0) {
        ...
        $iNewUntill = strtotime((string) $this->_getArticleFieldValue($oArticle, 'wmdknewlabel'));
	...
	private function _getFirstActiveVariant($sArticleId = NULL, $iActive = 1, $iHidden = 0, $iMinStock = 0) {
        if ($this->_oFirstActiveVariant == NULL) {
            ...
            if ($oResult != FALSE && $oResult->count() > 0) {
                ...
                $this->_oFirstActiveVariant = ($oProduct->oxarticles__oxid->value != '') ? $oProduct : FALSE;
            }
        }
    ...
    private function _getArticleFieldValue($oArticle, $sFieldName, $mDefault = '')
    {
        if (!is_object($oArticle)) {
            return $mDefault;
        }

        $sProperty = 'oxarticles__' . strtolower($sFieldName);

        if (!isset($oArticle->{$sProperty}) || !is_object($oArticle->{$sProperty})) {
            return $mDefault;
        }
    ...
    public function test() {
        ...
        $iLang = Registry::getRequest()->getRequestParameter('lang');
