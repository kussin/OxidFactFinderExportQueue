...
{    
    ...
    private function _resetMissingProducts() {
        $aChannelList = $this->_getChannelList();

        if (count($aChannelList) === 0) {
            $this->_aResponse['system_errors'][] = 'No FACT Finder export channels are configured.';
            return;
        }
    ...
    {
        $aChannelList = array();
        $sChannelConfig = (new \Wmdk\FactFinderQueue\Service\ModuleSettingsReader())->getString('sWmdkFFGeneralChannelList');

        foreach (explode(',', $sChannelConfig) as $sChannel) {
            $aParams = explode('::', trim($sChannel));

            if (count($aParams) < 3 || $aParams[0] === '') {
                continue;
            }
