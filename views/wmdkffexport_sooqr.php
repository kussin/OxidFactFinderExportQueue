...
class wmdkffexport_sooqr extends oxubase
{
    ...
    private function _exportData() {
        // CONFIG
        $sExportFile = (new ExportDirectoryManager())->resolveConfiguredExportFile($this->_sChannel . '.sooqr.xml');
            ...
            foreach ($this->_aCsvData as $sRow) {
                ...
                if (count($aKeys) == count($aValues)) {
                    ...
                    $this->_aResponse['validation_errors'][] = sprintf(
                        'Sooqr export row has %d values for %d fields. Product row starts with: %s',
                        count($aValues),
                        count($aKeys),
                        substr($sRow, 0, 200)
                    );
