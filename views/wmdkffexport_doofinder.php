...
use Wmdk\FactFinderQueue\Service\GzipCompressor;
...
class wmdkffexport_doofinder extends oxubase
{
    ...
    private function _exportData() {
        // CONFIG
        $sExportFile = (new ExportDirectoryManager())->resolveConfiguredExportFile($this->_sChannel . '.doofinder.xml');
        ...
        try {
            ...
            if ($this->_validateXmlFile($sExportFile)) {
                // COMPRESS (.gz)
                $this->_aResponse['compressed_file'] = (new GzipCompressor())->compressFile($sExportFile);
...
//                    $aNode = $this->_replaceSpecialChars($aNode);
                    ...
                    $this->_aResponse['validation_errors'][] = sprintf(
                        'Doofinder export row has %d values for %d fields. Product row starts with: %s',
                        count($aValues),
                        count($aKeys),
                        substr($sRow, 0, 200)
                    );
